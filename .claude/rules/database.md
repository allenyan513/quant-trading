...
## 角色与最小权限（Phase A RLS 地基）

prod **按角色分权**，不再全用 owner（同一 `DATABASE_URL` 变量名、值按运行时不同）：

- **`qt_web_ro`** —— web dashboard + OAuth MCP **读**（`services/web/lib/db.ts`，经 `DATABASE_URL_WEB`）。只 `SELECT`，**读不到 `data_holdings_accounts.flex_token`**（列级 carve-out），不能写。
- **`qt_auth_rw`** —— web 的 Better Auth（`lib/auth-server.ts`），DML 仅限 7 张 `auth_*` 表。web 别的写全 forward 到 data（T12）。
- **`qt_service_rw`** —— data/alpha/portfolio（`@qt/shared` pg Pool，经 `DATABASE_URL`），DML on `data_*`/`alpha_*`/`portfolio_*`/`system_*`。
- **`neondb_owner`** —— 只用于**迁移**（DDL）与 **`pnpm db:grants`**（授权）。
- 授权真源:`scripts/db/grants.sql`（幂等,**不进迁移链**——drizzle 不建模 grants/roles;新表靠 `ALTER DEFAULT PRIVILEGES` 自动覆盖,新角色才需重跑 `db:grants`）。建角色 + 接线 + 验证见 `scripts/db/README.md`。本地 dev 可只用 owner(`DATABASE_URL_WEB` 不设 → 回退)。
- **全量行级 RLS**(`ENABLE/FORCE` + `app.user_id` 策略 + web 读改带 context 连接池)是 **Phase B**,见对应 issue;Phase A 只做角色分离。
