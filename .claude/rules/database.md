...
- **批量 upsert 引用待插入值用 `excluded.<列>`**：`onConflictDoUpdate({ set })` 的值不能引用某一行（批量时没有"那一行"），要用 `sql.raw(\`excluded.${col}\`)`。惯例：`const ex = (c: string) => sql.raw(\`excluded.${c}\`)`，再 `set: { navIndex: ex("nav_index") }`。参考 `services/portfolio/src/holdings/sync.ts`。
- **复合主键含可空列 → ON CONFLICT 不命中**：PG 把 NULL 视作互不相等，可空键列上的 upsert 会退化成重复插入/不更新。把这类键列设 `notNull().default(<哨兵>)`（`''` / `0` / `'1970-01-01'`），写入时显式填哨兵。见 `portfolio_holdings_positions` 的 `option_type`/`strike`/`expiry`。
...
- 跑前先 `SELECT` 一眼目标表有没有真实数据（`portfolio_holdings_*` 等实盘表尤其）；脚本用完即删，不要 commit。
