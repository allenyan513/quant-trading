...
四个 Hono 服务(data / alpha / portfolio / gateway)结构同构：`src/index.ts` 建 `Hono` app、挂端点、`serve({ fetch: app.fetch, port: config.port })`。改一个服务时参考其他服务的现有写法保持一致。(`services/spa` 不是 Hono —— 它是纯前端 SPA，见 spa.md。)
...
- `data_events` / `data_notifications` / `data_candidates` / `data_watchlist` / `data_watchlist_lists` / `data_news_items`（及其余 `data_*` 拉取表）← **data**。`data_watchlist_lists` = per-user 自选分组(tabs)，`data_watchlist.list_id` 指向它(可空,null=All)；gateway forward create/rename/delete/reorder/assign(#164)。
