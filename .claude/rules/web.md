...
- **MCP 端点**（`app/api/mcp/route.ts` → `/api/mcp`）：web 对第三方 LLM 托管 MCP（`mcp-handler`，streamable HTTP）。**开放无鉴权**（middleware 放行 `/api/mcp`）——为兼容 claude.ai 自定义连接器(只认 OAuth/免鉴权)。因此**只注册公开市场数据工具**（research + 两个 13F）；**私账户工具 `get_holdings` 绝不在此注册**（开放=泄露真实持仓,要它就走 OAuth 或单独 token 端点)。工具直读只读 DB（复用 `@qt/shared/research`、`@qt/shared/thirteenf-read`，注入 `lib/db.ts` 的 `db()`），**不回调 data**——MCP 全只读,与"web 只读 DB"一致。
