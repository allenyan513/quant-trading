...
- **Reference valuation（System A）**：慢、基本面驱动，只在财报/估值刷新时更新，持久化为不可变 `data_valuation_snapshots`（带 `code_version`）。**确定性引擎已迁到 data**（读 data 自有的 marketdata 缓存）；alpha 经 `POST {DATA_URL}/internal/valuation` 取回作为重定价输入（`src/reference-valuation.ts`）。
