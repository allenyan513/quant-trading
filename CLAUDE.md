...
- `services/web` —— Next.js 只读仪表盘 + **唯一对外公开入口**。也托管 **MCP 端点**(`app/api/mcp/route.ts` → `/api/mcp`，streamable HTTP，`mcp-handler`)向第三方 LLM 吐**公开市场数据**(`get_symbol_research` / `list_13f_investors` / `get_13f_investor`，均 SEC/公开源)。**端点开放无鉴权**——为兼容 claude.ai 自定义连接器(只认 OAuth 或免鉴权,不收静态 bearer);故**私账户工具 `get_holdings` 不在此注册**(开放会泄露真实持仓;导出逻辑留在 `lib/mcp/holdings.ts`,以后经 OAuth 或单独 token 端点再启用)。**MCP 工具直读只读 DB**(复用 `@qt/shared/research`、`@qt/shared/thirteenf-read`，与仪表盘同源,不回调 data)。data/alpha/portfolio 为**内部服务**。
