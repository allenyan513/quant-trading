...
- `services/web` —— Next.js 只读仪表盘 + **唯一对外公开入口**。也托管 **MCP 端点**(`app/api/[transport]/route.ts` → `/api/mcp`，streamable HTTP，bearer `MCP_TOKEN` 门控、fail-closed)向第三方 LLM 吐数据(`get_symbol_research` / `get_holdings` / `list_13f_investors` / `get_13f_investor`)；**MCP 工具直读只读 DB**(复用 `@qt/shared/research`、`@qt/shared/thirteenf-read`，与仪表盘同源,不回调 data)。data/alpha/portfolio 为**内部服务**。
