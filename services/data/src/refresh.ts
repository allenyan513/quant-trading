/**
 * Per-symbol / watchlist data refresh — the server-side equivalent of the
 * dashboard's "刷新数据" button (which orchestrates the same two owners, T12).
 *
 *   warm marketdata caches (data) + pull recent news (data) + recompute the
 *   reference valuation (alpha /internal/valuation).
 *
 * Used by three entry points:
 *  - the per-symbol `/warm` endpoint (web button) — warm + news only,
 *  - auto-refresh when a symbol is added to the watchlist — warm + news awaited,
 *    valuation fired best-effort (the daily sweep is the backstop),
 *  - the daily `/jobs/refresh-watchlist` cron — full refresh of every symbol.
 *
 * Every step is isolated: one failed dataset (e.g. a premium-gated statement, or
 * alpha being briefly unreachable) must never abort the rest.
 */
import { mapLimit } from "@qt/shared";
import { warmSymbol } from "./warm.js";
import { pullSymbolNews } from "./pull/news-feed.js";
import { stageNews } from "./news.js";
import { getWatchlistSymbols } from "./watchlist.js";
import { computeReferenceValuation } from "./valuation/reference.js";
import { log } from "./log.js";

const msg = (err: unknown) => (err instanceof Error ? err.message : String(err));

/** Max symbols refreshed concurrently by the watchlist job. Bounds FMP/alpha
 *  fan-out; fmpGet still throttles globally underneath. */
const REFRESH_CONCURRENCY = 2;

export interface RefreshResult {
  symbol: string;
  warmed: boolean;
  newsInserted: number;
  revalued: boolean;
}

/** Warm the symbol's marketdata caches + pull its recent news. Deterministic
 *  (no LLM), typically a few seconds. Each step isolated. */
export async function warmAndPullNews(
  symbol: string,
): Promise<{ symbol: string; warmed: boolean; newsPulled: number; newsInserted: number }> {
  const sym = symbol.toUpperCase();
  let warmed = false;
  try {
    await warmSymbol(sym);
    warmed = true;
  } catch (err) {
    log.warn("refresh.warm_failed", { symbol: sym, error: msg(err) });
  }

  // Market-wide /news/pull leaves single tickers stale, so pull this symbol's
  // news explicitly. Isolated: a news failure must not fail the marketdata warm.
  let newsPulled = 0;
  let newsInserted = 0;
  try {
    const items = await pullSymbolNews(sym, { days: 30 });
    newsPulled = items.length;
    newsInserted = (await stageNews(items)).inserted;
  } catch (err) {
    log.warn("refresh.news_failed", { symbol: sym, error: msg(err) });
  }

  return { symbol: sym, warmed, newsPulled, newsInserted };
}

/** Recompute the reference valuation (deterministic, data-local since the engine
 *  moved here from alpha). forceRefresh: the caller just warmed the marketdata.
 *  Best-effort: never throws. */
export async function revalue(symbol: string): Promise<boolean> {
  const sym = symbol.toUpperCase();
  try {
    await computeReferenceValuation(sym, { forceRefresh: true });
    return true;
  } catch (err) {
    log.warn("refresh.revalue_failed", { symbol: sym, error: msg(err) });
    return false;
  }
}

/** Full per-symbol refresh: warm + news + valuation, all awaited. */
export async function refreshSymbol(symbol: string): Promise<RefreshResult> {
  const { symbol: sym, warmed, newsInserted } = await warmAndPullNews(symbol);
  const revalued = await revalue(sym);
  return { symbol: sym, warmed, newsInserted, revalued };
}

/** Refresh every watchlist symbol (daily cron). Bounded concurrency. */
export async function refreshWatchlist(): Promise<{ scanned: number; results: RefreshResult[] }> {
  const symbols = await getWatchlistSymbols();
  const results = await mapLimit(symbols, REFRESH_CONCURRENCY, (s) => refreshSymbol(s));
  return { scanned: symbols.length, results };
}
