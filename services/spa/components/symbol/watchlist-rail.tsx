...
export function WatchlistRail({ symbol }: { symbol: string }) {
  ...
  // The active tab segment within /workspace/data/symbol/:symbol/:seg → keep it when
  // switching symbols. Path indices: [workspace, data, symbol, :symbol, :seg].
  const seg = useLocation().pathname.split("/").filter(Boolean)[4] ?? "chart";
