 ...
 * public market-data (get_symbol_research + two 13F tools + search_sec_filings, from
 ...
 * WWW-Authenticate pointing at /.well-known/oauth-protected-resource → Claude runs the
 * OAuth dance (DCR + PKCE) against the AS and retries with a bearer.
 *
 * Differences from the Next version: no `export const runtime/dynamic`, no GET/POST/
 * DELETE route exports — `src/index.ts` mounts the exported `mcpRequestHandler` on a
 * Hono `app.all("/mcp", ...)`. basePath is "" (was "/api") so it serves at /mcp.
 * `createMcpHandler`/`withMcpAuth` return Web-standard `(req: Request) => Promise<Response>`.
 */
import { createMcpHandler, withMcpAuth } from "mcp-handler";
import { z } from "zod";
import { auth } from "../auth.js";
import { registerPublicTools } from "./register-public-tools.js";
import { getHoldingsExport, HOLDINGS_SECTIONS } from "./holdings.js";
import { listWatchlistOverview } from "../queries/index.js";
...
// The gateway is read-only DB (T12): WRITE tools forward to the owning service
// (data owns memos/morning-briefs; portfolio owns the paper account).
const mcpHandler = createMcpHandler(
  ...
  },
  {
    ...
    // Claude's connector card). Without this the server reads as an anonymous set of
    // tools — set it to brand the connector.
  ...
  // Rootless on the api subdomain (was "/api" in web) ⇒ served at /mcp.
  { basePath: "" },
);
...
/** OAuth-gated MCP request handler — `app.all("/mcp", c => mcpRequestHandler(c.req.raw))`. */
export const mcpRequestHandler = withMcpAuth(mcpHandler, verifyToken, {
  required: true,
