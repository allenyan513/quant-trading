/**
 * Server-side forwarder to the portfolio service (mirror of `data-proxy.ts`). web
 * is read-only on the DB, so any write the portfolio service owns — placing a paper
 * order, resetting the account — is forwarded here. The service URL is read as a
 * static `process.env.PORTFOLIO_URL` (Next only inlines static reads; `config.*()`
 * resolves empty at runtime). Unwraps the `ok(data)` / `fail(code, msg)` envelope.
 */
const TIMEOUT_MS = 10_000;

export function portfolioUrl(): string {
  const u = process.env.PORTFOLIO_URL;
  if (!u) throw new Error("Missing required env var: PORTFOLIO_URL");
  return u;
}
