...
function ValuationCard({ symbol, v }: { symbol: string; v: Overview["valuation"] }) {
  ...
  return (
    <Link href={`/workspace/data/symbol/${symbol}/valuation`} style={cardLink}>
...
function PositionCard({ p }: { p: NonNullable<Overview["positions"]> }) {
  const open = p[0];
  return (
    <Link href="/workspace/portfolio/positions" style={cardLink}>
...
function NewsCard({ symbol, news }: { symbol: string; news: NonNullable<Overview["news"]> }) {
  return (
    <Link href={`/workspace/data/symbol/${symbol}/news`} style={cardLink}>
...
function RatiosCard({ symbol, ratios }: { symbol: string; ratios: Overview["ratios"] }) {
  ...
  return (
    <Link href={`/workspace/data/symbol/${symbol}/financials`} style={cardLink}>
