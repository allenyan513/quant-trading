...
// ---------- render: forward estimates (prominent, top of page) ----------
function ForwardEstimates({ income, estimates }: { income: Row[]; estimates: Row[] }) {
  const lastDate = income.at(-1)?.fiscalDate ?? "";
  ...
  return (
    <Card title="前瞻预期 · 分析师一致预期" accent="#a371f7">
      <Grid min={190}>
        {rows.map((r) => (
          <div key={r.year} style={{ border: "1px solid var(--border)", borderRadius: 8, padding: "10px 12px" }}>
            <div style={{ fontSize: 12, color: "#a371f7", fontWeight: 700, marginBottom: 6 }}>FY{r.year}E{r.an != null ? ` · ${r.an} 分析师` : ""}</div>
            <div style={{ display: "grid", gap: 4, fontSize: 13 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                <span style={{ color: "var(--muted)" }}>营收</span>
      ...
      <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 8 }}>FMP 仅提供营收与 EPS 的前瞻一致预期；同样并入下方利润表的 <span style={{ color: "#a371f7" }}>E</span> 列。</div>
    </Card>
  );
}
